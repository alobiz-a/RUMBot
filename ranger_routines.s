...
interrupt_rise:
    ...
    bsf	    CCP4IE 	; Re-enable the CCP4 interrupt (INTERRUPT ENABLE bit) 
    bcf	    CCP4IF
    retfie	f
...
interrupt_fall:
    ...
    call    display_results ; Lights up a port for debugging purposes
    
    retfie  f    

display_results:    ;*****The pins that light up correspond to the 8 bit value stored in each variable***
    movlw   0x00
    movwf   TRISC
    movwf   TRISD
    movff   time_H, LATC
...
flash_rise: ;*****For debugging: PORTA pins light up when there has been an interrupt on the RISING edge******
    clrf    LATA
    movlw   0x00
...
flash_fall:;*****For debugging: PORTA pins light up when there has been an interrupt on the FALLING edge******
    clrf    LATA
    movlw   0x00
    movwf   TRISA	; Make all pins on PORTB outputs
