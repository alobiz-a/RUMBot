...
global  LCD_Setup, LCD_Write_Message, ClearLCD, LCD_Send_Byte_I, LCD_Send_Byte_D
...
LCD_Send_Byte_I:	    ; Transmits byte stored in W to instruction reg
	...
	bcf	LATB, LCD_RS, A	; Instruction write CLEAR RS bit (indicates the byte is an INSTRUCTION)
...
LCD_Send_Byte_D:	    ; Transmits byte stored in W to data reg
	...
	andlw   0x0f	    ; select just low nibble
	movwf   LATB, A	    ; output data bits to LCD
	bsf	LATB, LCD_RS, A	; Data write SET RS bit (indicates byte should be DISPLAYED)
	call    LCD_Enable  ; Pulse enable Bit 
	movf	LCD_tmp, W, A	; swap nibbles, now do low nibble
	andlw   0x0f	    ; select just low nibble
	movwf   LATB, A	    ; output data bits to LCD
	bsf	LATB, LCD_RS, A	; Data write set RS bit	    
        call    LCD_Enable  ; Pulse enable Bit 
	movlw	10	    ; delay 40us - ADDITIONAL DELAY: needed to ensure enough time to process and display character
	call	LCD_delay_x4us
	return
...
ClearLCD:
	call	LCD_delay_ms
	movlw	0x01	; clears lcd
	call	LCD_Send_Byte_I	;   check if there are further instructions
